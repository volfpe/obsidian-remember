...
import type { CardKind, ParsedSibling } from './card-note';
...
export interface NoteCard {
	...
	suspended: boolean;
	reverse: boolean;
	/** Independently scheduled prompts generated by this note. */
	siblings: ParsedSibling[];
	/** 0-based editor line of the card's first content. */
	line: number;
	path: string;
	deck: string;
}
...
/** The root deck '' contains every deck. */
export function isDescendantDeck(deck: string, ancestor: string): boolean {
	return ancestor === '' || deck === ancestor || deck.startsWith(ancestor + '/');
}
