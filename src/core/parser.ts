...
function parseBlock(lines: readonly MarkdownLine[], start: number, end: number, out: ParsedCard[]): void {
	...
	if (separator >= 0) {
		...
		const front = blockText(lines, contentStart, separator - 1);
	...
	for (let i = start; i <= end; i++) {
		...
		const source = withoutTokens(raw, searchable);
		...
		const front = stripTokens(raw.slice(0, at), searchable.slice(0, at)).trim();
		const back = stripTokens(raw.slice(at + width), searchable.slice(at + width)).trim();
		out.push({
			id: endToken ? endToken[1] : null,
			kind: 'basic',
